...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    for i in range(T):
        ...
        # if all_correct:
        #     return ths/(), th0s/()
    return ths/(data.shape[0]*T), th0s/(data.shape[0]*T)
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    ...
    return result/labels_test.shape[1]
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        ...
        perform = eval_classifier(learner, train_data, train_labels, train_data, train_labels)
        sum_performances += perform
    return sum_performances/it
...
def xval_learning_alg(learner, data, labels, k):
    ...
    for i, fold in enumerate(folds):
        folds_minus = folds[:i] + folds[i + 1:]
        labels_minus = label_folds[:i] + label_folds[i+1:]
        D_minus_i = np.concatenate(folds_minus, axis=1)
        lables_minus_i = np.concatenate(labels_minus, axis=1)
        score = eval_classifier(learner, D_minus_i, lables_minus_i, fold, label_folds[i])
        scores += score
    return scores/k
